...
Interactive implementation of solving the gridworld environment using Q-learning/Sarsa
--- for testing and visualising different action policies and knowledge updation
...
USAGE: If rl_lib_simple package installed, run 'qlearn-gridworld-gui' from command line.
        Else, run python test_qlearn_interactive.py

GUI created by modifying the GUI generated py PAGE version 4.10
...
class QLearnerParameters:
    ...
    def collect_obstacle_list_from_user(self):
        ...
        if self.custom_set == False:
            ...
            if obst_records is not None:
                ...
                print "setting custom obstacles"
            else:
                print "obstacles not changed"

        else:
            self.custom_obstacles = None
            print "setting random obstacles"
        print self.custom_obstacles
    ...
    def buttonSTARTpressed(self):
        ...
        print self.row_out, self.col_out,self.start_row_out,self.start_col_out,self.goal_row_out,self.goal_col_out, self.check_vis_out, self.lr_out, self.df_out, self.eps_out, self.act_pol, self.val_update_pol
...
if __name__ == '__main__':
    ...
    if root is None:
        env = GridWorldEnv(grid_row = learner_params_.row_out, grid_col = learner_params_.col_out, 
                           ...
                           list_of_hole_pos = learner_params_.custom_obstacles,
